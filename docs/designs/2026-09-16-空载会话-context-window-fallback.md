...
# 订正：分母单线化 [用户裁决 2026-09-17]

本方案的「`usage.model` 优先」旁路链已被**分母单线化**取代（2.3.2 ②）。裁决要点（用户原话）：「那就完全依赖 claudian对模型的配置」「我选什么模型，显示什么模型的上下文长度才对」。

- 新数据流单线：**模型选择器当前模型 → 该模型 preset contextWindow → 分母（未配置则 200k）**。`usage.model` 是 runtime 标签（CLI 回报形态如 `claude-sonnet[1m]`），不再参与分母——它带前缀映射不到 preset 键，2.3.1 旁路实测会静默落 200k。
- live 路径同步订正（超出本方案范围，见 [[2026-09-15-claudian-B4双轨统一与DOM窗口化整合实施方案]] §2.2 A0b 标注）：result 的 `contextWindow` 对分母不采信。
- 下文「推荐改动 1」的候选优先序、「模型来源优先序」与「验证方式 3」按此裁决理解为历史方案；`refreshUsageContextWindow` 现只读选择器模型（`selectorModel`）单候选。runtime 权威窗口保留仅服务非 Claude provider（Codex/ACP）。
...
1. 在 `/Users/vincentwang/Documents/NoteVault/tools/claudian/src/features/chat/utils/usageInfo.ts` 增加一个薄的“按 provider 配置刷新 usage”公共函数（或等价集中逻辑）：
   - 候选模型优先 `usage.model`；缺失时才用调用方提供的当前/草稿模型。〔订正 [用户裁决 2026-09-17]：候选已改为调用方传入的选择器模型单线，`usage.model` 不再参与——见顶部订正段。〕
...
模型来源优先序：runtime 权威窗口（同模型） > 已持久化 `usage.model` 对应 preset 窗口 > 当前 tab/provider 模型对应 preset 窗口 > 200k。〔订正 [用户裁决 2026-09-17]：优先序已单线化为——选择器模型 preset 窗口 > 200k；runtime 权威窗口仅剩非 Claude provider。〕真正从未发送的新 tab 没有 usage，meter 本就隐藏；首次发送由既有 turn model snapshot/session_init 链建立 usage。
...
3. 补 `/Users/vincentwang/Documents/NoteVault/tools/claudian/tests/unit/features/chat/ClaudianView.test.ts`：修改 preset window 后，历史 tab 按自身 usage.model 刷新，不被当前全局模型改名；权威窗口保留。〔订正 [用户裁决 2026-09-17]：刷新已改为按选择器模型（全局当前模型）统一重算并重标——「我选什么模型，显示什么模型的上下文长度才对」。〕
