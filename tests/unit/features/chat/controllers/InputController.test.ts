...
describe('InputController - Message Queue', () => {
  ...
  describe('Sending messages', () => {
    ...
    it('generates a single-source turnId and threads it into the ChatTurnRequest', async () => {
      deps = createSendableDeps();
      (deps as any).mockAgentService.query = jest.fn().mockImplementation(() => createMockStream([{ type: 'done' }]));
      inputEl = deps.getInputEl() as ReturnType<typeof createMockInputEl>;
      inputEl.value = 'hello with turn id';
      controller = new InputController(deps);

      await controller.sendMessage();

      const prepareTurn = (deps as any).mockAgentService.prepareTurn as jest.Mock;
      expect(prepareTurn).toHaveBeenCalledTimes(1);
      const request = prepareTurn.mock.calls[0][0];
      // Non-empty, unique per send, generated by the existing id generator.
      expect(typeof request.turnId).toBe('string');
      expect(request.turnId.length).toBeGreaterThan(0);
      // The prepared turn (echoed through the real encoder mock) keeps it.
      const query = (deps as any).mockAgentService.query as jest.Mock;
      const preparedTurn = query.mock.calls[0][0];
      expect(preparedTurn.turnId).toBe(request.turnId);
      expect(preparedTurn.request.turnId).toBe(request.turnId);
    });

    it('generates a fresh turnId for each subsequent send', async () => {
      deps = createSendableDeps();
      (deps as any).mockAgentService.query = jest.fn().mockImplementation(() => createMockStream([{ type: 'done' }]));
      inputEl = deps.getInputEl() as ReturnType<typeof createMockInputEl>;
      controller = new InputController(deps);
      ...
      const firstTurnId = prepareTurn.mock.calls[0][0].turnId;
      const secondTurnId = prepareTurn.mock.calls[1][0].turnId;
